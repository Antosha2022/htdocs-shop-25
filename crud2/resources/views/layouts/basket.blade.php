    ...
    <div class="dropdown-menu">
            ...
            @foreach(session('cart') as $id => $details)
                <div class="row cart-detail">
                    <div class="col-lg-4 col-sm-4 col-4 cart-detail-img">
                        <img src="/mediaFiles/img_product{id}.png" />
                    </div>
                    <div class="col-lg-8 col-sm-8 col-8 cart-detail-product">
                        <p>{{ $details['short_name'] }}</p>
                        <span class="price text-info"> ${{ $details['retail_price'] }}</span> <span class="count"> Quantity:{{ $details['quantity'] }}</span>
                    </div>
                </div>
            @endforeach
        @endif
        <div class="row">
            <div class="col-lg-12 col-sm-12 col-12 text-center checkout">
                <a href="{{ route('cart') }}" class="btn btn-outline-info btn-block">View all</a>
            </div>
        </div>
    </div>
</div>
